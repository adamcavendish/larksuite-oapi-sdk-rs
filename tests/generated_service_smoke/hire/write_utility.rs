...
async fn hire_write_utility_query_smoke() {
    ...
    .await
    .unwrap();
    Box::pin(hire.interviewer.patch(
        "interviewer-1",
    ...
    .await
    .unwrap();
    Box::pin(hire.note.create(
        json!({"talent_id":"talent-1","content":"created"}),
        &RequestOption::default(),
    ))
    .await
    .unwrap();
    Box::pin(hire.note.patch(
        "note-1",
    ...
    .await
    .unwrap();
    Box::pin(hire.attachment.create(&RequestOption::default()))
        .await
        .unwrap();
    Box::pin(hire.website_channel.create(
        "website-1",
    ...
    .await
    .unwrap();
    Box::pin(hire.website_channel.update(
        "website-1",
        "channel-1",
