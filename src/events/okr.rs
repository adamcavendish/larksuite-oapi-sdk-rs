...
impl EventDispatcher {
    pub fn on_p2_okr_okr_updated_v1<F, Fut>(self, handler: F) -> Self
    where
        F: Fn(P2OkrOkrUpdatedV1) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = Result<()>> + Send + 'static,
    {
        self.on_event("okr.okr.updated_v1", wrap_handler(handler))
    }
}
