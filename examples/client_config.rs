...
use larksuite_oapi_sdk_rs::{LARK_BASE_URL, LarkClient};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let default_client = LarkClient::builder(&app_id, &app_secret).build()?;
    ...
    let tuned_client = LarkClient::builder(app_id, app_secret)
