// generate_go_compatibility_metadata extracts the Go SDK endpoint delta used by GoCompatibility.
package main
...
func fail(err error) {
	fmt.Fprintln(os.Stderr, "generate_go_compatibility_metadata:", err)
	os.Exit(1)
}
...
func render(endpoints []endpoint, from, to string) []byte {
	...
	fmt.Fprintln(&output, "// Code generated by tools/generate_go_compatibility_metadata.go; DO NOT EDIT.")
	...
	fmt.Fprintln(&output, "pub enum GoCompatibilityEndpoint {")
	...
	fmt.Fprintln(&output, "pub struct GoCompatibilityEndpointMeta {")
	...
	fmt.Fprintln(&output, "}")
	fmt.Fprintln(&output)
	fmt.Fprintln(&output, "impl GoCompatibilityEndpoint {")
	...
	fmt.Fprintln(&output, "    ];")
	fmt.Fprintln(&output)
	fmt.Fprintln(&output, "    pub fn meta(self) -> GoCompatibilityEndpointMeta {")
	fmt.Fprintln(&output, "        match self {")
	for _, endpoint := range endpoints {
		fmt.Fprintf(&output, "            Self::%s => GoCompatibilityEndpointMeta {\n", endpointName(endpoint))
